 ...
 */
  ...
  http
      .createServer(function (req, res) {
          ...
          if (pathname === "/") {
              ...
              return;
          } else if (req.method === "POST" && pathname === "/upload-file") { // ACCEPT USER INPUT
          ...
          } else if (pathname === "/auth/google/callback") {  // RECEIVE CODE ENDPOINT
  ...
  function handleFileUpload(req, hash, callback) {
      ...
      let filename = hash + '-' + req.headers['filename'];
  
      if (filename == null) { // no file name provided just use hash
          filename = "file." + hash + '-' + req.headers['content-type'].split('/')[1]
      }
